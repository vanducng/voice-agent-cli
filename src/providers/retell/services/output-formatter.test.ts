...
function makeSdkError(type: { prototype: Error }, message: string): Error {
  const error = Object.create(type.prototype) as Error;
  Object.defineProperty(error, "message", { value: message });
  return error;
}

function captureError(run: () => never) {
  const stderr = vi.spyOn(console, "error").mockImplementation(() => undefined);

  expect(run).toThrow(ReportedCliError);
...
describe("agent-friendly output", () => {
  ...
  it("redacts credential-shaped values from local error messages", () => {
    const response = captureError(() =>
      outputError(
        'Invalid JSON near {"apiKey":"local-secret-marker"}; RETELL_API_KEY=env-secret-marker; Authorization: Bearer bearer-secret-marker',
        "INVALID_JSON",
      ),
    );
    const output = JSON.stringify(response);

    expect(output).not.toContain("local-secret-marker");
    expect(output).not.toContain("env-secret-marker");
    expect(output).not.toContain("bearer-secret-marker");
    expect(output).toContain("[REDACTED]");
  });

  it("reports errors without forcing process exit before stderr flushes", () => {
    const stderr = vi
      .spyOn(console, "error")
    ...
    expect(() => outputError("Invalid value", "VALIDATION_ERROR")).toThrow(
      ReportedCliError,
    );
  ...
  it.each([
    [Retell.NotFoundError, "NOT_FOUND", false],
    [Retell.AuthenticationError, "AUTH_ERROR", false],
    [Retell.BadRequestError, "BAD_REQUEST", false],
    [Retell.PermissionDeniedError, "PERMISSION_DENIED", false],
    [Retell.RateLimitError, "RATE_LIMIT", true],
    [Retell.InternalServerError, "SERVER_ERROR", true],
    [Retell.APIConnectionTimeoutError, "TIMEOUT_ERROR", true],
    [Retell.APIConnectionError, "CONNECTION_ERROR", true],
    [Retell.APIError, "API_ERROR", false],
  ])("maps %s to %s", (type, code, retryable) => {
    const response = captureError(() =>
      handleSdkError(makeSdkError(type, "provider-secret-marker")),
    );
    ...
    expect(response.error.next_steps.length).toBeGreaterThan(0);
    expect(JSON.stringify(response)).not.toContain("provider-secret-marker");
  });

  it.each([
    ["NO_CONFIG", "vac retell login"],
    ["INVALID_CONFIG", "Fix or remove"],
    ["INVALID_JSON", "Fix or remove"],
    ["WRITE_ERROR", "permissions"],
  ])("guides ConfigError %s", (code, guidance) => {
    const response = captureError(() =>
      handleSdkError(
        new ConfigError("Config failure at /tmp/.voice-agent.json", code),
      ),
    );

    expect(response.error).toMatchObject({ code, retryable: false });
    expect(response.error.next_steps.join(" ")).toContain(guidance);
  });

  it("adds ok only to message-style successes", () => {
    const stdout = vi.spyOn(console, "log").mockImplementation(() => undefined);

    outputSuccess({ message: "Saved", id: "agent_1" });
